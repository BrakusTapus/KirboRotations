...
internal class MCHLogic : MCH_KirboBeta
{
    ...
    internal bool DoPrePullSteps(out IAction act)
    {
        ...
        if (CurrentState == OpenerState.PrePull && PrePullStep > 0)
        {
            ...
            if (Configs.GetCombo("RotationSelection") == 0)
            {
                ...
                if (Player.HasStatus(true, StatusID.Reassemble) && PrePullStep == 1) CurrentState = OpenerState.InOpener;
                // Otherwise, if it's the first pre-pull step, set the action to Reassemble.
                else if (PrePullStep == 1) Reassemble.CanUse(out act, CanUseOption.MustUseEmpty);
            ...
            if (Configs.GetCombo("RotationSelection") == 1)
            {
                ...
                if (IsLastGCD(ActionID.HeatedSplitShot) && PrePullStep == 1) CurrentState = OpenerState.InOpener;
                // Otherwise, if it's the first pre-pull step, set the action to SplitShot.
                else if (PrePullStep == 1) SplitShot.CanUse(out act, CanUseOption.MustUseEmpty);
            }
    ...
    private bool DoOpener(out IAction act)
    {
        ...
        if (currentState == OpenerState.InOpener)
        {
            if (Configs.GetCombo("RotationSelection") == 0)
            {
                if (IsLastAction(ActionID.AirAnchor) && OpenerStep == 1) OpenerStep++;
                else if (OpenerStep == 1) AirAnchor.CanUse(out act);

                if (IsLastAction(ActionID.GaussRound) && OpenerStep == 2) OpenerStep++;
                else if (OpenerStep == 2) GaussRound.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction(ActionID.Ricochet) && OpenerStep == 3) OpenerStep++;
                else if (OpenerStep == 3) Ricochet.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction(ActionID.Drill) && OpenerStep == 4) OpenerStep++;
                else if (OpenerStep == 4) Drill.CanUse(out act);

                if (IsLastAction(ActionID.BarrelStabilizer) && OpenerStep == 5) OpenerStep++;
                else if (OpenerStep == 5) BarrelStabilizer.CanUse(out act);

                if (IsLastAction(ActionID.HeatedSplitShot) && OpenerStep == 6) OpenerStep++;
                else if (OpenerStep == 6) SplitShot.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction(ActionID.HeatedSlugShot) && OpenerStep == 7) OpenerStep++;
                else if (OpenerStep == 7) SlugShot.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction(ActionID.GaussRound) && OpenerStep == 8) OpenerStep++;
                else if (OpenerStep == 8) GaussRound.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction(ActionID.Ricochet) && OpenerStep == 9) OpenerStep++;
                else if (OpenerStep == 9) Ricochet.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction((ActionID)ActionIDs.HeatedCleanShot) && OpenerStep == 10) OpenerStep++;
                else if (OpenerStep == 10) CleanShot.CanUse(out act);
                ...
                else if (OpenerStep == 12) ChainSaw.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction((ActionID)ActionIDs.AutomatonQueen) && OpenerStep == 13) OpenerStep++;
                else if (OpenerStep == 13) RookAutoturret.CanUse(out act);
                ...
                else if (OpenerStep == 24) Ricochet.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction(ActionID.Drill) && OpenerStep == 25) OpenerStep++;
                else if (OpenerStep == 25) Drill.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction(ActionID.GaussRound) && OpenerStep == 26) OpenerStep++;
                else if (OpenerStep == 26) GaussRound.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction(ActionID.Ricochet) && OpenerStep == 27) OpenerStep++;
                else if (OpenerStep == 27) Ricochet.CanUse(out act, CanUseOption.MustUseEmpty);
            ...
            {
                if (IsLastAction(ActionID.HeatedSplitShot) && OpenerStep == 1) OpenerStep++;
                else if (OpenerStep == 1) SplitShot.CanUse(out act, CanUseOption.MustUseEmpty);
                ...
                if (IsLastAction(ActionID.HeatedSlugShot) && OpenerStep == 6) OpenerStep++;
                else if (OpenerStep == 6) SlugShot.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction(ActionID.Ricochet) && OpenerStep == 7) OpenerStep++;
                else if (OpenerStep == 7) Ricochet.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction((ActionID)ActionIDs.HeatedCleanShot) && OpenerStep == 8) OpenerStep++;
                else if (OpenerStep == 8) CleanShot.CanUse(out act);

                if (IsLastAction(ActionID.Reassemble) && OpenerStep == 9) OpenerStep++;
                else if (OpenerStep == 9) Reassemble.CanUse(out act);

                if (IsLastAction(ActionID.GaussRound) && OpenerStep == 10) OpenerStep++;
                else if (OpenerStep == 10) GaussRound.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction(ActionID.AirAnchor) && OpenerStep == 11) OpenerStep++;
                else if (OpenerStep == 11) AirAnchor.CanUse(out act);

                if (IsLastAction(ActionID.Reassemble) && OpenerStep == 12) OpenerStep++;
                else if (OpenerStep == 12) Reassemble.CanUse(out act);

                if (IsLastAction(ActionID.Wildfire) && OpenerStep == 13) OpenerStep++;
                else if (OpenerStep == 13) Wildfire.CanUse(out act);

                if (IsLastAction(ActionID.ChainSaw) && OpenerStep == 14) OpenerStep++;
                else if (OpenerStep == 14) ChainSaw.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction((ActionID)ActionIDs.AutomatonQueen) && OpenerStep == 15) OpenerStep++;
                else if (OpenerStep == 15) RookAutoturret.CanUse(out act);
                ...
                else if (OpenerStep == 26) Ricochet.CanUse(out act, CanUseOption.MustUseEmpty);

                if (IsLastAction(ActionID.Drill) && OpenerStep == 27) CurrentState = OpenerState.OpenerFinished;
                else if (OpenerStep == 27) Drill.CanUse(out act);
            }
    ...
    public bool DoFullOpener(out IAction act)
    {
        ...
        if (CurrentState == OpenerState.PrePull)
            if (DoPrePullSteps(out act)) return true;

        if (CurrentState == OpenerState.InOpener)
        {
            if (DoOpener(out act)) return true;
        }
