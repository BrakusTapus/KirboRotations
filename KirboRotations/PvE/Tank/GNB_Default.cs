...
internal sealed class GNB_DefaultPvE : GNB_Base
{
    ...
    {
        if (remainTime <= 0.7 && LightningShot.CanUse(out var act))
        {
            return act;
        }
    ...
    protected override bool EmergencyAbility(IAction nextGCD, out IAction act)
    {
        ...
        {
            if (!CombatElapsedLessGCD(2) && NoMercy.CanUse(out act, CanUseOption.MustUse | CanUseOption.IgnoreClippingCheck))
            {
                return true;
            }

            if (Player.HasStatus(true, StatusID.NoMercy) && BloodFest.CanUse(out act, CanUseOption.MustUse | CanUseOption.IgnoreClippingCheck))
            {
                return true;
            }
        }
    ...
    {
        if (FatedCircle.CanUse(out act, aoeCount: 4))
        {
            return true;
        }

        if (CanUseGnashingFang(out act))
        {
            return true;
        }

        if (FatedCircle.CanUse(out act))
        {
            return true;
        }

        if (DemonSlaughter.CanUse(out act))
        {
            return true;
        }

        if (DemonSlice.CanUse(out act))
        {
            return true;
        }

        if (Player.HasStatus(true, StatusID.NoMercy) && CanUseSonicBreak(out act))
        {
            return true;
        }

        if (Player.HasStatus(true, StatusID.NoMercy) && CanUseDoubleDown(out act))
        {
            return true;
        }

        if (SavageClaw.CanUse(out act, CanUseOption.EmptyOrSkipCombo))
        {
            return true;
        }

        if (WickedTalon.CanUse(out act, CanUseOption.EmptyOrSkipCombo))
        {
            return true;
        }

        if (CanUseBurstStrike(out act))
        {
            return true;
        }

        if (SolidBarrel.CanUse(out act))
        {
            return true;
        }

        if (BrutalShell.CanUse(out act))
        {
            return true;
        }

        if (KeenEdge.CanUse(out act))
        {
            return true;
        }
        ...
        if (LightningShot.CanUse(out act))
        {
            return true;
        }
    ...
    private static bool CanUseGnashingFang(out IAction act)
    {
        if (GnashingFang.CanUse(out act))
        {
            ...
            if (Ammo == MaxAmmo && (Player.HasStatus(true, StatusID.NoMercy) || !NoMercy.WillHaveOneCharge(55)))
            {
                return true;
            }

            if (Ammo > 0 && !NoMercy.WillHaveOneCharge(17) && NoMercy.WillHaveOneCharge(35))
            {
                return true;
            }

            if (Ammo == 3 && IsLastGCD((ActionID)BrutalShell.ID) && NoMercy.WillHaveOneCharge(3))
            {
                return true;
            }

            if (Ammo == 1 && !NoMercy.WillHaveOneCharge(55) && BloodFest.WillHaveOneCharge(5))
            {
                return true;
            }

            if (Ammo == 1 && !NoMercy.WillHaveOneCharge(55) && (!BloodFest.IsCoolingDown && BloodFest.EnoughLevel || !BloodFest.EnoughLevel))
            {
                return true;
            }
        }
    ...
    private static bool CanUseSonicBreak(out IAction act)
    {
        if (SonicBreak.CanUse(out act))
        {
            ...
            if (!GnashingFang.EnoughLevel && Player.HasStatus(true, StatusID.NoMercy))
            {
                return true;
            }

            if (GnashingFang.IsCoolingDown && Player.HasStatus(true, StatusID.NoMercy))
            {
                return true;
            }
