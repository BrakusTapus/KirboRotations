...
internal class PLD_DefaultPvE : PLD_Base
{
    ...
    protected override bool AttackAbility(out IAction act)
    {
        ...
        if (InCombat)
        {
            ...
            if (IsBurst && !CombatElapsedLess(5) && FightOrFlight.CanUse(out act, CanUseOption.OnLastAbility))
            {
                return true;
            }
        }
        ...
        if (CircleOfScorn.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }

        if (SpiritsWithin.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }

        if (Player.WillStatusEndGCD(6, 0, true, StatusID.FightOrFlight)
            && Requiescat.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }
        ...
        if (!IsMoving && Intervene.CanUse(out act, option))
        {
            return true;
        }

        if (HasTankStance && OathGauge == 100 && UseOath(out act))
        {
            return true;
        }
    ...
    protected override bool GeneralGCD(out IAction act)
    {
        if (Player.HasStatus(true, StatusID.Requiescat))
        {
            ...
            {
                if (Player.HasStatus(true, StatusID.ConfiteorReady))
                {
                    return true;
                }

                if (Confiteor.ID != Confiteor.AdjustedID)
                {
                    return true;
                }
            }
            if (HolyCircle.CanUse(out act))
            {
                return true;
            }

            if (HolySpirit.CanUse(out act))
            {
                return true;
            }
        }
        ...
        //AOE
        if (HasDivineMight && HolyCircle.CanUse(out act))
        {
            return true;
        }

        if (Prominence.CanUse(out act))
        {
            return true;
        }

        if (TotalEclipse.CanUse(out act))
        {
            return true;
        }

        //Single
        if (!CombatElapsedLess(8) && HasFightOrFlight && GoringBlade.CanUse(out act))
        {
            return true; // Dot
        }
        ...
        if (!FightOrFlight.WillHaveOneChargeGCD(2))
        {
            if (!FightOrFlight.WillHaveOneChargeGCD(6) &&
                HasDivineMight && HolySpirit.CanUse(out act))
            {
                return true;
            }

            if (RageOfHalone.CanUse(out act))
            {
                return true;
            }

            if (Atonement.CanUse(out act))
            {
                return true;
            }
        }
        ...
        if (RiotBlade.CanUse(out act))
        {
            return true;
        }

        if (FastBlade.CanUse(out act))
        {
            return true;
        }
    ...
    {
        if (PassageOfArms.CanUse(out act))
        {
            return true;
        }
    ...
    {
        if (Sheltron.CanUse(out act, option))
        {
            return true;
        }

        if (Intervention.CanUse(out act, option))
        {
            return true;
        }
