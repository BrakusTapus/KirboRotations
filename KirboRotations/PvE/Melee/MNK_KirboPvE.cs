...
internal sealed class MNK_KirboPvE : MNK_Base
{
    ...
    protected override IAction CountDownAction(float remainTime)
    {
        if (remainTime < 0.2)
        {
            if (Thunderclap.CanUse(out var act, CanUseOption.IgnoreClippingCheck))
            {
                return act;
            }
        }
        if (remainTime < 15)
        {
            if (Chakra < 5 && Meditation.CanUse(out var act, CanUseOption.IgnoreClippingCheck))
            {
                return act;
            }

            if (FormShift.CanUse(out act, CanUseOption.IgnoreClippingCheck))
            {
                return act;
            }
        }
    ...
    {
        if (PerfectBalanceActions(out act))
        {
            return true;
        }
        ...
        if (Chakra < 5 && Meditation.CanUse(out act))
        {
            return true;
        }

        if (Configs.GetBool("AutoFormShift") && FormShift.CanUse(out act))
        {
            return true;
        }
    ...
    private static bool PerfectBalanceActions(out IAction act)
    {
        if (!BeastChakras.Contains(BeastChakra.NONE))
        {
            ...
            {
                if (PhantomRush.CanUse(out act, CanUseOption.MustUse))
                {
                    return true;
                }

                if (TornadoKick.CanUse(out act, CanUseOption.MustUse))
                {
                    return true;
                }
            }
            ...
            {
                if (RisingPhoenix.CanUse(out act, CanUseOption.MustUse))
                {
                    return true;
                }

                if (FlintStrike.CanUse(out act, CanUseOption.MustUse))
                {
                    return true;
                }
            }
            ...
            {
                if (ElixirField.CanUse(out act, CanUseOption.MustUse))
                {
                    return true;
                }
            }
    ...
    private static bool SolarNadi(out IAction act)
    {
