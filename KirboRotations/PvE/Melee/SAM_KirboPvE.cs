...
internal sealed class SAM_KirboPvE : SAM_Base
{
    ...
    {
        //奥义回返
        if (KaeshiNamikiri.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }
        ...
        //燕回返
        if (KaeshiGoken.CanUse(out act, CanUseOption.MustUse | CanUseOption.EmptyOrSkipCombo))
        {
            return true;
        }

        if (KaeshiSetsugekka.CanUse(out act, CanUseOption.MustUse | CanUseOption.EmptyOrSkipCombo))
        {
            return true;
        }
        ...
        if ((!IsTargetBoss || (HostileTarget?.HasStatus(true, StatusID.Higanbana) ?? false)) && HasMoon && HasFlower
            && OgiNamikiri.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }
        ...
        {
            if (HasMoon && HasFlower && Higanbana.CanUse(out act))
            {
                return true;
            }
        }
        if (SenCount == 2)
        {
            if (TenkaGoken.CanUse(out act, !MidareSetsugekka.EnoughLevel ? CanUseOption.MustUse : CanUseOption.None))
            {
                return true;
            }
        }
        if (SenCount == 3)
        {
            if (MidareSetsugekka.CanUse(out act))
            {
                return true;
            }
        }
        ...
        //连击2
        if ((!HasMoon || IsMoonTimeLessThanFlower || !Oka.EnoughLevel) && Mangetsu.CanUse(out act, HaveMeikyoShisui && !HasGetsu ? CanUseOption.EmptyOrSkipCombo : CanUseOption.None))
        {
            return true;
        }

        if ((!HasFlower || !IsMoonTimeLessThanFlower) && Oka.CanUse(out act, HaveMeikyoShisui && !HasKa ? CanUseOption.EmptyOrSkipCombo : CanUseOption.None))
        {
            return true;
        }

        if (!HasSetsu && Yukikaze.CanUse(out act, HaveMeikyoShisui && HasGetsu && HasKa && !HasSetsu ? CanUseOption.EmptyOrSkipCombo : CanUseOption.None))
        {
            return true;
        }

        //连击3
        if (Gekko.CanUse(out act, HaveMeikyoShisui && !HasGetsu ? CanUseOption.EmptyOrSkipCombo : CanUseOption.None))
        {
            return true;
        }

        if (Kasha.CanUse(out act, HaveMeikyoShisui && !HasKa ? CanUseOption.EmptyOrSkipCombo : CanUseOption.None))
        {
            return true;
        }

        //连击2
        if ((!HasMoon || IsMoonTimeLessThanFlower || !Shifu.EnoughLevel) && Jinpu.CanUse(out act))
        {
            return true;
        }

        if ((!HasFlower || !IsMoonTimeLessThanFlower) && Shifu.CanUse(out act))
        {
            return true;
        }
        ...
        {
            //连击1
            if (Fuko.CanUse(out act))
            {
                return true;
            }

            if (!Fuko.EnoughLevel && Fuga.CanUse(out act))
            {
                return true;
            }

            if (Hakaze.CanUse(out act))
            {
                return true;
            }

            //燕飞
            if (Enpi.CanUse(out act))
            {
                return true;
            }
        }
