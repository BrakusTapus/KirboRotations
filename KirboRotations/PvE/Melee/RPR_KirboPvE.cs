...
internal sealed class RPR_KirboPvE : RPR_Base
{
    ...
    protected override IAction CountDownAction(float remainTime)
    {
        if (remainTime < Harpe.CastTime + CountDownAhead
            && Harpe.CanUse(out var act))
        {
            return act;
        }

        if (SoulSow.CanUse(out act))
        {
            return act;
        }
    ...
    protected override bool GeneralGCD(out IAction act)
    {
        ...
        if (WhorlOfDeath.CanUse(out act))
        {
            return true;
        }

        if (ShadowOfDeath.CanUse(out act))
        {
            return true;
        }
        ...
        if (HasEnshrouded)
        {
            ...
            if (LemureShroud > 1)
            {
                ...
                {
                    if (ShadowOfDeath.CanUse(out act, CanUseOption.MustUse))
                    {
                        return true;
                    }
                }
            ...
            if (LemureShroud == 1)
            {
                if (Communio.EnoughLevel)
                {
                    ...
                    {
                        if (ShadowOfDeath.CanUse(out act, IsMoving ? CanUseOption.MustUse : CanUseOption.None))
                        {
                            return true;
                        }
                    }
        ...
        if (!CombatElapsedLessGCD(2) && PlentifulHarvest.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }

        if (SoulScythe.CanUse(out act, CanUseOption.EmptyOrSkipCombo))
        {
            return true;
        }

        if (SoulSlice.CanUse(out act, CanUseOption.EmptyOrSkipCombo))
        {
            return true;
        }

        if (NightmareScythe.CanUse(out act))
        {
            return true;
        }

        if (SpinningScythe.CanUse(out act))
        {
            return true;
        }

        if (InfernalSlice.CanUse(out act))
        {
            return true;
        }

        if (WaxingSlice.CanUse(out act))
        {
            return true;
        }

        if (Slice.CanUse(out act))
        {
            return true;
        }

        if (InCombat && HarvestMoon.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }

        if (Harpe.CanUse(out act))
        {
            return true;
        }
