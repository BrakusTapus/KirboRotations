...
internal sealed class RDM_KirboPvE : RDM_Base
{
    ...
    protected override IAction CountDownAction(float remainTime)
    {
        if (remainTime < VerthunderStartUp.CastTime + CountDownAhead
            && VerthunderStartUp.CanUse(out var act, CanUseOption.EmptyOrSkipCombo))
        {
            return act;
        }
    ...
    protected override bool EmergencyGCD(out IAction act)
    {
        if (ManaStacks == 3)
        {
            ...
            {
                if (Verholy.CanUse(out act, CanUseOption.MustUse))
                {
                    return true;
                }
            }
            if (Verflare.CanUse(out act, CanUseOption.MustUse))
            {
                return true;
            }
        }

        if (Resolution.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }

        if (Scorch.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }

        if (IsLastGCD(true, Moulinet) && Moulinet.CanUse(out act, CanUseOption.MustUse))
        {
            return true;
        }

        if (Zwerchhau.CanUse(out act))
        {
            return true;
        }

        if (Redoublement.CanUse(out act))
        {
            return true;
        }
        ...
        {
            if (BlackMana >= 50 && WhiteMana >= 50 && Riposte.CanUse(out act))
            {
                return true;
            }
        }
        if (ManaStacks > 0 && Riposte.CanUse(out act))
        {
            return true;
        }
