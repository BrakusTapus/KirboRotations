...
[BetaRotation]
[SourceCode(Path = "main/KirboRotations/Magical/BLM_Default.cs")]
internal class BLM_KirboPvE : BLM_Base
{
    ...
    protected override bool AttackAbility(out IAction act)
    {
        ...
        if (InUmbralIce)
        {
                ...
                && !IsLastGCD(ActionID.Paradox))
            {
                ...
                if (TripleCast.CanUse(out act, CanUseOption.EmptyOrSkipCombo))
                {
                    return true;
                }
            }

            if (UmbralIceStacks < 3 && LucidDreaming.CanUse(out act))
            {
                return true;
            }

            if (SharpCast.CanUse(out act, CanUseOption.EmptyOrSkipCombo))
            {
                return true;
            }
        }
        if (InAstralFire)
        {
            if (!CombatElapsedLess(6) && CombatElapsedLess(9) && LeyLines.CanUse(out act))
            {
                return true;
            }

            if (TripleCast.CanUse(out act, gcdCountForAbility: 5))
            {
                return true;
            }
        }
        if (Amplifier.CanUse(out act))
        {
            return true;
        }
    ...
    protected override IAction CountDownAction(float remainTime)
    {
        ...
        {
            if (Fire3.CanUse(out act))
            {
                return act;
            }
        }
        if (remainTime <= 12 && SharpCast.CanUse(out act, CanUseOption.EmptyOrSkipCombo | CanUseOption.IgnoreClippingCheck))
        {
            return act;
        }
    ...
    protected override bool EmergencyAbility(IAction nextGCD, out IAction act)
    {
        ...
        if (InAstralFire)
        {
            if (CurrentMp == 0 && Manafont.CanUse(out act))
            {
                return true;
            }

            //To Ice
            if (NeedToTransposeGoIce(true) && Transpose.CanUse(out act))
            {
                return true;
            }
        }
    ...
    protected override bool GeneralGCD(out IAction act)
    {
        ...
        if (IsMoving && HasHostilesInRange && TripleCast.CanUse(out act, CanUseOption.EmptyOrSkipCombo | CanUseOption.IgnoreClippingCheck))
        {
            return true;
        }
    ...
    {
        if (BetweenTheLines.CanUse(out act))
        {
            return true;
        }

        if (LeyLines.CanUse(out act))
        {
            return true;
        }
    ...
    private static bool NeedToGoIce
    {
        ...
        {
            //Can use Despair.
            if (Despair.EnoughLevel && CurrentMp >= Despair.MPNeed)
            {
                return false;
            }

            //Can use Fire1
            if (Fire.EnoughLevel && CurrentMp >= Fire.MPNeed)
            {
                return false;
            }
    ...
    private static bool AddElementBase(out IAction act)
    {
        ...
        {
            if (Blizzard2.CanUse(out act))
            {
                return true;
            }

            if (Blizzard3.CanUse(out act))
            {
                return true;
            }
    ...
    private static bool AddThunder(out IAction act, uint gcdCount = 3)
    {
        ...
        //Return if just used.
        if (IsLastGCD(ActionID.Thunder, ActionID.Thunder2, ActionID.Thunder3, ActionID.Thunder4))
        {
            return false;
        }
        ...
        if (Thunder2.CanUse(out act))
        {
            return true;
        }

        if (Thunder.CanUse(out act))
        {
            return true;
        }
    ...
    private static bool DoFire(out IAction act)
    {
        ...
        if (TripleCast.CanUse(out act, CanUseOption.IgnoreClippingCheck))
        {
            return true;
        }
        ...
        if (UmbralHearts < 2 && Flare.CanUse(out act))
        {
            return true;
        }
    ...
    private static bool DoIce(out IAction act)
    {
        if (IsLastAction(ActionID.UmbralSoul, ActionID.Transpose)
            && IsParadoxActive && Blizzard.CanUse(out act))
        {
            return true;
        }
        ...
        {
            if (Freeze.CanUse(out act))
            {
                return true;
            }

            if (Blizzard4.CanUse(out act))
            {
                return true;
            }
        }
    ...
    private static bool MaintainIce(out IAction act)
    {
        ...
        if (UmbralIceStacks == 1)
        {
            ...
            if (Player.Level == 90 && Blizzard.CanUse(out act))
            {
                return true;
            }
    ...
    private static bool NeedToTransposeGoIce(bool usedOne)
    {
        ...
        if (count == compare++ && !EnchinaEndAfterGCD(2))
        {
            return false;
        }

        if (count >= compare && (HasFire || Swiftcast.WillHaveOneChargeGCD(2) || TripleCast.WillHaveOneChargeGCD(2)))
        {
            return true;
        }

        if (!HasFire && !Swiftcast.WillHaveOneChargeGCD(2) && !TripleCast.CanUse(out _, gcdCountForAbility: 8))
        {
            return false;
        }
    ...
    private static bool UsePolyglot(out IAction act, uint gcdCount = 3)
    {
        ...
        {
            if (Foul.CanUse(out act))
            {
                return true;
            }

            if (Xenoglossy.CanUse(out act))
            {
                return true;
            }
        }
    ...
    private bool MaintainFire(out IAction act)
    {
        switch (AstralFireStacks)
        {
            case 1:
                ...
                {
                    if (HasFire && Fire3.CanUse(out act))
                    {
                        return true;
                    }

                    if (IsParadoxActive && Fire.CanUse(out act))
                    {
                        return true;
                    }
                }
        ...
        {
            if (CurrentMp >= Fire.MPNeed * 2 + 800 && Fire.CanUse(out act))
            {
                return true;
            }
    ...
    private bool MaintainStatus(out IAction act)
    {
        ...
        if (UmbralSoul.CanUse(out act))
        {
            return true;
        }

        if (InAstralFire && Transpose.CanUse(out act))
        {
            return true;
        }
