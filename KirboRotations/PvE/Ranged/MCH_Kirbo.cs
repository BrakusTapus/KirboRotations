...
internal class MCH_Kirbo : MCH_Base
{
    ...
    {
        ActionCheck = (b, m) => (Player.HasStatus(true, StatusID.Overheated) && MCHLogic.HeatStacks > 4 || Heat >= 45) && InCombat
    };
            ...
            switch (Configs.GetCombo("RotationSelection"))
            {
                case 0: // Early AA
                    switch (OpenerHelpers.OpenerStep)
                    {
                        ...
                        case 16:
                            return OpenerHelpers.OpenerController(IsLastGCD(false, HeatBlast) && MCHLogic.HeatStacks == 4, HeatBlast.CanUse(out act, CanUseOption.MustUse));

                        case 17:
                            return OpenerHelpers.OpenerController(IsLastAbility(false, Ricochet), Ricochet.CanUse(out act, CanUseOption.MustUseEmpty));

                        case 18:
                            return OpenerHelpers.OpenerController(IsLastGCD(false, HeatBlast) && MCHLogic.HeatStacks == 3, HeatBlast.CanUse(out act, CanUseOption.MustUse));

                        case 19:
                            return OpenerHelpers.OpenerController(IsLastAbility(false, GaussRound), GaussRound.CanUse(out act, CanUseOption.MustUseEmpty));

                        case 20:
                            return OpenerHelpers.OpenerController(IsLastGCD(false, HeatBlast) && MCHLogic.HeatStacks == 2, HeatBlast.CanUse(out act, CanUseOption.MustUse));

                        case 21:
                            return OpenerHelpers.OpenerController(IsLastAbility(false, Ricochet), Ricochet.CanUse(out act, CanUseOption.MustUseEmpty));

                        case 22:
                            return OpenerHelpers.OpenerController(IsLastGCD(false, HeatBlast) && MCHLogic.HeatStacks == 1, HeatBlast.CanUse(out act, CanUseOption.MustUse));

                        case 23:
                            return OpenerHelpers.OpenerController(IsLastAbility(false, GaussRound), GaussRound.CanUse(out act, CanUseOption.MustUseEmpty));

                        case 24:
                            return OpenerHelpers.OpenerController(IsLastGCD(false, HeatBlast) && MCHLogic.HeatStacks == 0, HeatBlast.CanUse(out act, CanUseOption.MustUse));
        ...
        if (!OpenerHelpers.OpenerInProgress)
        {
            ...
            if (Reassemble.CanUse(out act, CanUseOption.MustUseEmpty))
            {
                if (Reassemble.CurrentCharges > 1)
                {
                    ...
                    {
                        Serilog.Log.Information($"{RotationConfigs.v} More then 1 Reassemble Charge. using Reassemble on any major action");
                        return true;
                    }
                }
            ...
            if (Hypercharge.CanUse(out act) && (IsLastAbility(ActionID.Wildfire) || !MCHLogic.WillhaveTool && (
    ...
    /* private bool UCoBRotation(IAction nextGCD, out IAction act)
      {
          ...
          {
              if (Battery == 100 || RatioOfMembersIn2minsBurst > 1 || BurstHelpers.InBurst)
                  return true;
          }
  ...
  */
    ...
    private void ToolKitCheck()
    {
        ...
        if (Player.Level >= 90)
        {
            ...
            MCHLogic.WillhaveTool = WillHaveDrill || WillHaveAirAnchor || WillHaveChainSaw;
        }
