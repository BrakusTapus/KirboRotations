...
internal class MCH_KirboPvE : MCH_Base
{
    ...
    private new static IBaseAction Dismantle
    {
        get;
    }
= new BaseAction(ActionID.Dismantle, ActionOption.None | ActionOption.Defense);
    ...
    {
        ActionCheck = (b, m) => (Player.HasStatus(true, StatusID.Overheated) &&
                                     MCHLogic.HeatStacks > 4 ||
                                 Heat >= 45) &&
                                InCombat
    };
    ...
    public override void DisplayStatus()
    {
        ...
        CompatibilityAndFeatures.AddUltimateCompatibility(
            UltimateCompatibility.UCoB);

        CompatibilityAndFeatures.AddContentCompatibility(
            ContentCompatibility.DutyRoulette);
        CompatibilityAndFeatures.AddContentCompatibility(
            ContentCompatibility.SavageRaids);
        CompatibilityAndFeatures.AddContentCompatibility(
            ContentCompatibility.ExtremeTrials);
        CompatibilityAndFeatures.AddContentCompatibility(
            ContentCompatibility.Criterion);
        CompatibilityAndFeatures.AddContentCompatibility(
            ContentCompatibility.Hunts);
        ...
        CompatibilityAndFeatures.CurrentRotationSelection =
            Configs.GetCombo("RotationSelection");

        try
        {
            DebugWindow.DisplayMCHTab();
            ImGui.SameLine();
    ...
    protected override IAction CountDownAction(float remainTime)
    {
        ...
        // If 'OpenerActionsAvailable' is true (see method
        // 'HandleOpenerAvailability' for conditions) proceed to using Action logic
        // during countdown
        if (OpenerHelpers.OpenerActionsAvailable)
        {
            // Selects action logic depending on which rotation has been selected
            // (Default: Delayed Tool)
            switch (Configs.GetCombo("RotationSelection"))
            {
                case 0:  // Early AA
                         // Use Drill when the remaining countdown time is less or equal
                         // to Drill's AnimationLock, also sets OpenerInProgress to
                         // 'True'
                    if (remainTime <= AirAnchor.AnimationLockTime &&
                        AirAnchor.CanUse(out _))
                    {
                        OpenerHelpers.OpenerInProgress = true;
                        return AirAnchor;
                    }
                    // Use Tincture if Tincture use is enabled and the countdown
                    // time is less or equal to SplitShot+Tincture animationlock
                    // (1.8s)
                    IAction act0;
                    if (remainTime <= AirAnchor.AnimationLockTime +
                ...
                case 1:  // Delayed Tools
                         // Use SplitShot when the remaining countdown time is less or
                         // equal to SplitShot's AnimationLock, also sets
                         // OpenerInProgress to 'True'
                    if (remainTime <= SplitShot.AnimationLockTime &&
                        SplitShot.CanUse(out _))
                    {
                        OpenerHelpers.OpenerInProgress = true;
                        return SplitShot;
                    }
                    ...
                    // case 2: // Early All
                    //     if (remainTime <= AirAnchor.AnimationLockTime &&
                    //     Player.HasStatus(true, StatusID.Reassemble) &&
                    //     AirAnchor.CanUse(out _))
                    //     {
        ...
        if (Player.Level < 90)
        {
            if (AirAnchor.EnoughLevel && remainTime <= 0.6 + CountDownAhead &&
                AirAnchor.CanUse(out _))
            {
                return AirAnchor;
            }
            if (!AirAnchor.EnoughLevel && Drill.EnoughLevel &&
                remainTime <= 0.6 + CountDownAhead && Drill.CanUse(out _))
            {
                return Drill;
            }
            if (!AirAnchor.EnoughLevel && !Drill.EnoughLevel &&
                HotShot.EnoughLevel && remainTime <= 0.6 + CountDownAhead &&
                HotShot.CanUse(out _))
            {
                return HotShot;
            }
            if (!AirAnchor.EnoughLevel && !Drill.EnoughLevel &&
                !HotShot.EnoughLevel && remainTime <= 0.6 + CountDownAhead &&
                CleanShot.CanUse(out _))
            {
                return CleanShot;
            }
        ...
        if (UltimateRaids)
        {
            if (UwUorUCoB)
            {
                if (remainTime <= Drill.AnimationLockTime &&
                    Player.HasStatus(true, StatusID.Reassemble) &&
                    Drill.CanUse(out _))
                {
                    return Drill;
        ...
        while (OpenerHelpers.OpenerInProgress)
        {
            switch (Configs.GetCombo("RotationSelection"))
            {
                case 0:  // Early AA
                    ...
                    {
                        case 0:
                            return OpenerHelpers.OpenerController(
                                IsLastGCD(false, AirAnchor),
                                AirAnchor.CanUse(out act, CanUseOption.MustUse));

                        case 1:
                            return OpenerHelpers.OpenerController(
                                IsLastAbility(false, GaussRound),
                                GaussRound.CanUse(out act,
                                                  CanUseOption.MustUseEmpty));

                        case 2:
                            return OpenerHelpers.OpenerController(
                                IsLastAbility(false, Ricochet),
                                Ricochet.CanUse(out act,
                                                CanUseOption.MustUseEmpty));

                        case 3:
                            return OpenerHelpers.OpenerController(
                                IsLastGCD(false, Drill),
                                Drill.CanUse(out act, CanUseOption.MustUse));

                        case 4:
                            return OpenerHelpers.OpenerController(
                                IsLastAbility(false, BarrelStabilizer),
                                BarrelStabilizer.CanUse(out act,
                                                        CanUseOption.MustUse));

                        case 5:
                            return OpenerHelpers.OpenerController(
                                IsLastGCD(true, SplitShot),
                                SplitShot.CanUse(out act, CanUseOption.MustUse));
                        ...
                        case 7:
                            return OpenerHelpers.OpenerController(
                                IsLastGCD(true, SlugShot),
                                SlugShot.CanUse(out act, CanUseOption.MustUse));
                        ...
                        case 10:
                            return OpenerHelpers.OpenerController(
                                IsLastGCD(true, CleanShot),
                                CleanShot.CanUse(out act, CanUseOption.MustUse));

                        case 11:
                            return OpenerHelpers.OpenerController(
                                IsLastAbility(false, Reassemble),
                                Reassemble.CanUse(out act,
                                                  CanUseOption.MustUseEmpty));

                        case 12:
                            return OpenerHelpers.OpenerController(
                                IsLastAbility(false, Wildfire),
                                Wildfire.CanUse(out act, (CanUseOption)17));

                        case 13:
                            return OpenerHelpers.OpenerController(
                                IsLastGCD(false, ChainSaw),
                                ChainSaw.CanUse(out act, CanUseOption.MustUse));

                        case 14:
                            return OpenerHelpers.OpenerController(
                                IsLastAbility(true, RookAutoturret),
                                RookAutoturret.CanUse(out act,
                                                      CanUseOption.MustUse));

                        case 15:
                            return OpenerHelpers.OpenerController(
                                IsLastAbility(false, Hypercharge),
                                Hypercharge.CanUse(out act, (CanUseOption)51));

                        case 16:
                            return OpenerHelpers.OpenerController(
                                IsLastGCD(false, HeatBlast) &&
                                    MCHLogic.HeatStacks == 4,
                                HeatBlast.CanUse(out act, CanUseOption.MustUse));
                        ...
                        case 27:
                            ...
                            // Serilog.Log.Information($"{v}
                            // {OpenerHelpers.OpenerComplete} - Early AA");
                            //  Finished Early AA
                            break;
                    }
                    break;
                ...
                case 1:  // Delayed Tools
                    switch (OpenerHelpers.OpenerStep)
                    {
                        ...
                        case 27:
                            ...
                            // OpenerHelpers.OpenerInProgress = false;
                            // Serilog.Log.Information($"{v}
                            // {OpenerHelpers.OpenerComplete} - Delayed Tools");
                ...
                case 2:  // Early All
                    switch (OpenerHelpers.OpenerStep)
                    {
                        ...
                        case 13:
                            return OpenerHelpers.OpenerController(
                                IsLastAbility(false, Tactician),
                                Tactician.CanUse(out act,
                                                 CanUseOption.MustUseEmpty));
    ...
    protected override bool GeneralGCD(out IAction act)
    {
        ...
        if (!OpenerHelpers.OpenerInProgress)
        {
            if (AutoCrossbow.CanUse(out act, (CanUseOption)1, 2) &&
                HostileTarget.DistanceToPlayer() <= 12f)
            {
                return true;
            }
            ...
            if (HostileTarget.GetHealthRatio() > 0.6 && IsLongerThan(15) &&
                BioBlaster.CanUse(out act, (CanUseOption)1, 2) &&
                HostileTarget.DistanceToPlayer() <= 12f)
            {
                return true;
            }
            ...
            if (!AirAnchor.EnoughLevel &&
                HotShot.CanUse(out act, CanUseOption.MustUse) && !IsOverheated)
            {
                return true;
            }
    ...
    protected override bool EmergencyAbility(IAction nextGCD, out IAction act)
    {
        ...
        if (Configs.GetBool("BatteryStuck") && Battery == 100 &&
            RookAutoturret.CanUse(out act, CanUseOption.MustUseEmpty) &&
            (nextGCD == ChainSaw || nextGCD == AirAnchor || nextGCD == CleanShot))
        {
            return true;
        }
        if (Configs.GetBool("HeatStuck") && Heat == 100 &&
            Hypercharge.CanUse(out act, CanUseOption.MustUseEmpty) &&
            (nextGCD == SplitShot || nextGCD == SlugShot || nextGCD == CleanShot))
        {
            return true;
        }
        ...
        // UCoB
        TerritoryContentType Content = TerritoryContentType;
        bool UwUorUCoB = (int)Content == 28 && Player.Level == 70;
        var logic = new MCHLogic();
        if (UwUorUCoB)
        {
            return logic.UCoBRotation(nextGCD, out act);
        }
        ...
        if (!OpenerHelpers.OpenerInProgress)
        {
                ...
                if (Reassemble.CurrentCharges > 1)
                {
                        ...
                        nextGCD == Drill)
                    {
                        Serilog.Log.Information(
                            $"{RotationConfigs.v} More then 1 Reassemble Charge. using Reassemble on any major action");
                        return true;
                    }
                }
            ...
            if (RookAutoturret.CanUse(out act, (CanUseOption)16) && HostileTarget &&
                HostileTarget.IsTargetable && InCombat)
            {
                ...
                // if (Wildfire.IsCoolingDown && Wildfire.ElapsedAfter(105f) &&
                // Battery == 100 && (nextGCD == AirAnchor || nextGCD == CleanShot))
                //{
                //     return true;
                // }
            ...
            // when using delayed tools, hypercharge should be used 3 GCD's before
            // dril comes up at the 2min mark. Currently drifts barrel stabilizer
            // and overcaps 20 heat because of it link
            // 'https://xivanalysis.com/fflogs/a:K6Jnpwbv4z3WRyGL/1/1' 12.230dps
            // 10min test when using Early AA, misses crowned colider during 2nd
            // tincture, loses 30 battery due to overcap at 2min mark loses 10
            // battery as queen is not used at 90 battery which then gets followed
            // by AA at 4min 15s loses another 20 battery when gauge is at 100
            // shouldve used queen after the clean shot at 4min 7s, 8s before
            // wildfire came off cooldown link
            // 'https://xivanalysis.com/fflogs/a:K6Jnpwbv4z3WRyGL/3/1' 12.100dps
            // 10min test in both tests there's a weaving issue at 8min 46 (either
            // gauss or rico's problem, maybe implement oGCD counter or something)
            if (Hypercharge.CanUse(out act) &&
                (IsLastAbility(ActionID.Wildfire) ||
                 !MCHLogic.WillhaveTool &&
                     (BurstHelpers.InBurst &&
                          IsLastGCD(ActionID.ChainSaw, ActionID.AirAnchor,
                                    ActionID.Drill, ActionID.SplitShot,
                                    ActionID.SlugShot, ActionID.CleanShot,
                                    ActionID.HeatedSplitShot,
                                    ActionID.HeatedSlugShot) ||
                      Heat >= 100 && Wildfire.WillHaveOneCharge(10f) ||
                      Heat >= 100 && Wildfire.WillHaveOneCharge(
                                         40f) ||  // was 90 (causes issues with 2min
                                                  // early aa burst)
            ...
            if (ShouldUseGaussroundOrRicochet(out act) &&
                WeaponRemain > GaussRound.AnimationLockTime + Ping)
            {
                return true;
            }
        }
        ...
        if (Player.Level < 90)
        {
            ...
            if (Hypercharge.CanUse(out act) && InCombat && HostileTarget &&
    ...
    /* private bool UCoBRotation(IAction nextGCD, out IAction act)
      {
          act = null;
          ...
          if (Wildfire.CanUse(out act, (CanUseOption)16))
          {
              if (Heat >= 50 ||
                  ...
                  Heat >= 45 && !Drill.WillHaveOneCharge(5) &&
      !HotShot.WillHaveOneCharge(7.5f))
              {
                  return true;
              }
          }

          if (RookAutoturret.CanUse(out act) && HostileTarget.GetHealthRatio() > 0.1
      && HostileTarget.IsTargetable)
          {
              if (Battery == 100 || RatioOfMembersIn2minsBurst > 1 ||
      BurstHelpers.InBurst) return true;
          }

          if (HostileTarget.GetHealthRatio() > 0.1 && HostileTarget.IsTargetable)
          {
          }
          if (Hypercharge.CanUse(out act) && (IsLastAbility(ActionID.Wildfire) ||
      !WillhaveTool && ( BurstHelpers.InBurst && IsLastGCD(ActionID.Drill,
      ActionID.SplitShot, ActionID.SlugShot, ActionID.CleanShot,
      ActionID.HeatedSplitShot, ActionID.HeatedSlugShot) || Heat >= 100 &&
      Wildfire.WillHaveOneCharge(10f) || Heat >= 100 &&
      Wildfire.WillHaveOneCharge(40f) || Heat >= 50 &&
      !Wildfire.WillHaveOneCharge(40f)
          )))
          {
              return true;
          }
          ...
          if (Reassemble.CanUse(out act, CanUseOption.MustUseEmpty) && nextGCD ==
      Drill)
          {
              return true;
          }

          if (ShouldUseGaussroundOrRicochet(out act) && WeaponRemain >
      GaussRound.AnimationLockTime + Ping)
          {
              return true;
          }
    ...
    */
    ...
    // This should be relevant to the Action shown in the
    // [RotationDesc(ActionID.Action]
    ...
    private void SaveAction()
    {
        ...
        {
            BattleCharaEx.SaveAction = false;
        }

        if (UwUorUCoB && saveneeded)
        {
            BattleCharaEx.SaveAction = true;
        }
    }
    ...
    private bool ShouldUseBurstMedicine(out IAction act)
    {
        ...
        if (Player.HasStatus(true, StatusID.Weakness) ||
            Player.HasStatus(true, (StatusID)Buffs.Transcendent))
        {
            return false;
        }
        ...
        // at some point) Drill's CD is 3s or less
        if (Wildfire.WillHaveOneCharge(20) && CombatTime > 60 &&
            ...
            !TinctureOfDexterity8.IsCoolingDown && Drill.WillHaveOneCharge(3))
        {
    ...
    private bool ShouldUseGaussroundOrRicochet(out IAction act)
    {
        ...
        // First, check if both GaussRound and Ricochet do not have at least one
        ...
        // Fourth, check if Ricochet has more or an equal number of charges compared
        // to GaussRound. If so, prefer using Ricochet.
    ...
    {
        if (!Player.HasStatus(true, StatusID.Reassemble) &&
            Reassemble.CanUse(out act, (CanUseOption)2) &&
            Reassemble.CurrentCharges > 0 &&
            (nextGCD == ChainSaw || nextGCD == AirAnchor || nextGCD == Drill))
        {
            return true;
        }
        ...
        if (Hypercharge.CanUse(out act) && !MCHLogic.WillhaveTool && Heat >= 50)
        {
            return true;
        }
        if (HostileTarget.GetHealthRatio() < 0.03 && nextGCD == CleanShot &&
            Reassemble.CurrentCharges > 0 &&
            Reassemble.CanUse(out act, CanUseOption.IgnoreClippingCheck))
        {
            return true;
        }
        if (HostileTarget.GetHealthRatio() < 0.03 &&
            RookAutoturret.ElapsedAfter(5f) && QueenOverdrive.CanUse(out act))
        {
            return true;
        }
        if (HostileTarget.GetHealthRatio() < 0.02 &&
            (Player.HasStatus(true, StatusID.Wildfire) || BurstHelpers.InBurst) &&
            Wildfire.ElapsedAfter(5f) && Detonator.CanUse(out act))
        {
            return true;
        }
        return false;
    ...
    private void ToolKitCheck()
    {
        ...
        {
            // Player is level 76 or higher but lower than 90, check Drill and Air
            // Anchor
            MCHLogic.WillhaveTool = WillHaveDrill || WillHaveAirAnchor;
        }
        else if (Player.Level >= 58)
        {
            // Player is level 58 or higher but lower than 76, check only Drill
            MCHLogic.WillhaveTool = WillHaveDrill;
        }
    ...
    private void HandleOpenerAvailability()
    {
        ...
        OpenerHelpers.OpenerActionsAvailable =
        ...
        OpenerHelpers.LvL70_Ultimate_OpenerActionsAvailable =
            UwUorUCoB && NoResources && ReassembleOneCharge && HasDrill &&
            HasWildfire && HasBarrelStabilizer;

        OpenerHelpers.LvL80_Ultimate_OpenerActionsAvailable =
    ...
    // This method triggers when the game loads an instance or whenever the screen
    // fades to black
    ...
    public override void OnTerritoryChanged()
    {
        ...
        if (UwUorUCoB)
        {
            MCHLogic.InLvL70Ultimate = true;
        }
        else if (!UwUorUCoB)
        {
            MCHLogic.InLvL70Ultimate = false;
        }
